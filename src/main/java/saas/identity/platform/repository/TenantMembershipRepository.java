...
public interface TenantMembershipRepository extends JpaRepository<TenantMembershipEntity, UUID> {
   ...
   *
   * <p>status 列是 membership_status enum，native SQL 里 'active' 字面量需显式 cast： Hikari
   * stringtype=unspecified 只对 PreparedStatement 的参数绑定生效，对 @Query 内的硬编码字面量 不生效，所以查询中直接比较 'active'
   * 会触发「operator does not exist: membership_status = unknown」 错误（线上 500 复现）。显式 ::membership_status
   * 解决。
   */
  @Query(
      value =
